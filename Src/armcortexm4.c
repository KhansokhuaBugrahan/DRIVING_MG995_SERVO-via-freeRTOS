...
void nvicSetInterruptPriority(IRQ_NUMBER_e position, uint8_t priority )
{
	 uint8_t index = position / 4;
	 uint32_t bitShift = (position % 4) * 8;
	 NVIC_IPR->NVIC_IPR0[index] = priority << (4 + bitShift);
}
