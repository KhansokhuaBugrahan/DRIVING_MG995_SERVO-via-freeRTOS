...
#define    ADC_CENTER_DEAD_ZONE    15
...
void vMyTask3GeneratePulse(void* pvTask3 )
{
			...
			static int16_t previousPulseRead = 1400;
	...
	while(1)
	{
		...
		if(TIMER3_ENABLED())
		{
			 ...
			 emaFilteredPulse = previousPulseRead + ((pulseWidth - previousPulseRead) >> 2);
			...
			if(abs(delta) > 5)
			{
				 if(previousPulseRead <= 2500 && previousPulseRead >= 500)
				 {
					...
					{
						previousPulseRead = SERVO_MID_US;
						TIMER3->TIMx_CCR1 = SERVO_MID_US;
					}
